...
app.post("/login", (req, res) => {
  ...
  db.get(`SELECT * FROM users WHERE email = ?`, [email], async (err, row) => {
    ...
    console.log(
      `[AUTH REST] ✓ Login: User ${row.id}, Admin: ${!!row.is_admin}`
    );
    // include user_id in response so frontend can identify the logged-in user
    res.json({ token, user_id: row.id, admin: !!row.is_admin });
  });
});
